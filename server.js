  ...
  send({ text: text }, function(err, result) {
    ...
    if (err) {
      ...
      if (isXhr) {
        ...
        res.send(
          '<h1>An error has occurred</h1><p>Your message could not be sent.</p><p><a href="/">Back</a></p>'
        );
      }
    } else {
      if (isXhr) {
...
app.get("/instagram-auth", function(req, res) {
  res.send(
    "<h1>Instagram authentication</h1>" +
      '<p><a href="https://api.instagram.com/oauth/authorize/?client_id=' +
      config.feed.instagram.clientId +
      "&redirect_uri=" +
      encodeURIComponent(config.global.baseUrl) +
      '%2Finstagram-auth-landing&response_type=code">Click here</a></p>'
  );
});
...
app.get("/instagram-auth-landing", function(req, res) {
  ...
  request(options, function(error, response, body) {
    if (error) {
      ...
      res.send(
        "Paste this token into server-config.js @ feed.instagram.accessToken: <br />" +
          json.access_token
      );
    }
  });
